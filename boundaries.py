    ...
    def __init__(self, json_file):
        ...
        if not os.path.exists(json_file):
            raise FileNotFoundError(
                f"Файл {json_file} не найден. Сначала запустите 1_segmentation.py для создания суперпикселей.")
...
def main():
    ...
    if not args.input:
        ...
        if not available_files:
            ...
            print("Сначала запустите 1_segmentation.py для создания суперпикселей.")
            return
